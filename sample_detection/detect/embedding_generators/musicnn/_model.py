...
def batch_data(audio, sr, n_frames, overlap):
    ...
    - audio: audio for the music file to tag
    ...
    - overlap: amount of overlap (in frames) of the input spectrogram patches.
...
def extractor(
    ...
    n_frames = (
        librosa.time_to_frames(
            input_length, sr, n_fft=config.FFT_SIZE, hop_length=config.FFT_HOP
        )
        + 1
    )
    ...
    try:
        ...
        if model == "MSD_musicnn_big":
            ...
            raise ValueError(
                "MSD_vgg model is still training... will be available soon! :)"
            )
